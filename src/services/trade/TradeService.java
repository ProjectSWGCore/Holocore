...
public class TradeService extends Service {
	...
	private void handleSecureTrade(SecureTrade packet, Player player, ObjectManager objectManager) {
		...
		initiator.setTradePartner(accepter);
		handleTradeSessionRequest(packet, player, initiator, accepter, tradeSession);
	...
	private void handleAbortTradeMessage(Player player) {
		if(player.getCreatureObject().getObjectId() != player.getCreatureObject().getTradePartner().getObjectId()){
			player.getCreatureObject().getTradePartner().sendSelf(new AbortTradeMessage());
			player.getCreatureObject().getTradePartner().sendSelf(new DenyTradeMessage());
		} else {
	...
	private void handleAcceptTransactionMessage(Player player) {
		if(player.getCreatureObject().getObjectId() != player.getCreatureObject().getTradePartner().getObjectId()){
			player.getCreatureObject().getTradePartner().sendSelf(new AcceptTransactionMessage());
		} else{
	...
	private void handleUnAcceptTransactionMessage(Player player) {
		if(player.getCreatureObject().getObjectId() != player.getCreatureObject().getObjectId()){
			player.getCreatureObject().getTradePartner().sendSelf(new UnAcceptTransactionMessage());
		} else{
	...
	private void handleAddItemMessage(AddItemMessage packet, Player player) {
		...
		if(player.getCreatureObject().getObjectId() != player.getCreatureObject().getTradePartner().getObjectId()){
			player.getCreatureObject().getTradeSession().addToItemList(player.getCreatureObject().getTradePartner(), packet.getObjectId());
			for (long objectId : player.getCreatureObject().getTradePartner().getTradeSession().getFromAccepterList()) {
				player.getCreatureObject().sendSelf(new AddItemMessage(objectId));
			}
		} else{
			player.getCreatureObject().getTradePartner().getTradeSession().addToItemList(player.getCreatureObject(), packet.getObjectId());
			for (long objectId : player.getCreatureObject().getTradePartner().getTradeSession().getFromInitiatorList()) {
				player.getCreatureObject().getTradePartner().sendSelf(new AddItemMessage(objectId));
			}
		}
	}
	
	private void handleTradeSessionRequest(SecureTrade packet, Player packetSender , CreatureObject initiator, CreatureObject accepter, TradeSession tradeSession) {		
		...
		requestBox.addOkButtonCallback("handleTradeRequest", (player, actor, event, paramenters)-> {
			if(initiator.getTradePartner() != null && initiator.getTradePartner().equals(accepter) && accepter.getTradePartner() != null && accepter.getTradePartner().equals(initiator)){
				accepter.setTradeSession(tradeSession);
				accepter.setTradePartner(initiator);		
