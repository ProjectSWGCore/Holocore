...
class PatrolGroup(val count: Int, val formation: PatrolFormation) {
	private val _objects: MutableList<AIObject> = ArrayList()
	val objects: List<AIObject>
		get() = Collections.unmodifiableList(_objects)

	fun addObject(obj: AIObject) {
		_objects.add(obj)
	}
}
