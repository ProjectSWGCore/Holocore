...
class NpcWeaponRangeLoader : DataLoader() {
	private val weapons: MutableMap<String, Int> = HashMap()
	 ...
	 */
	fun getWeaponRange(weaponIff: String?): Int {
	...
	override fun load() {
		SdbLoader.load(File("serverdata/npc/npc_weapon.sdb")).use { set ->
			while (set.next()) {
				weapons[ClientFactory.formatToSharedFile(set.getText("weapons"))] = set.getInt("weapon_range").toInt()
			}
		}
	}
}
