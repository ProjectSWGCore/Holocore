...
class ActiveQuestRequirementParser : RequirementParser<ActiveQuestRequirement> {
	override fun parse(args: Map<String, Any>): ActiveQuestRequirement {
		val questName = args["quest"] as String?
		val active = args["active"] as Boolean
		val task = args["task"] as Long?

		return ActiveQuestRequirement(questName!!, active, getTaskValue(task))
	}

	private fun getTaskValue(task: Long?): Int? {
		if (task == null) {
