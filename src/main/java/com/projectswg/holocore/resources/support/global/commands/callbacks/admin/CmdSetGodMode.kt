...
class CmdSetGodMode : ICmdCallback {
	override fun execute(player: Player, target: SWGObject?, args: String) {
		val creatureObject = player.creatureObject

		if (creatureObject.hasCommand("admin")) {
			creatureObject.removeCommand("admin")
			broadcastPersonal(player, "God Mode Disabled")
		} else {
			creatureObject.addCommand("admin")
			broadcastPersonal(player, "God Mode Enabled")
		}
		ForceAwarenessUpdateIntent(creatureObject).broadcast()
	}
}
