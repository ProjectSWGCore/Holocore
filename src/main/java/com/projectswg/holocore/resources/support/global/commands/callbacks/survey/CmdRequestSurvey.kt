...
class CmdRequestSurvey : ICmdCallback {
	override fun execute(player: Player, target: SWGObject?, args: String) {
		if (target !is TangibleObject) return

		val resource = getGalacticResourceByName(args)
		if (resource == null) {
			broadcastPersonal(player, "Unknown resource: $args")
			return
		}
		if (player.creatureObject.hasCommand("admin")) {
			val spawns = ArrayList(resource.getSpawns(player.creatureObject.terrain))
			spawns.sortWith(Comparator.comparingDouble { c: GalacticResourceSpawn -> player.creatureObject.location.flatDistanceTo(c.getX(), c.getZ()) })
			for (spawn in spawns) {
				broadcastPersonal(player, "Spawn: $spawn")
			}
		}
		StartSurveyingIntent(player.creatureObject, resource).broadcast()
	}
}
