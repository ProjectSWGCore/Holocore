 ...
 * @author mads
 */
class RequestBiographyCmdCallback : ICmdCallback {
	override fun execute(player: Player, target: SWGObject?, args: String) {
		val creatureObject = player.creatureObject
		if (target != null) {
			if (target is CreatureObject && target.isPlayer) {
				player.sendPacket(BiographyUpdate(creatureObject.objectId, target.getObjectId(), target.playerObject.biography))
			} else {
