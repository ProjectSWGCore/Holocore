	...
	protected void handleCommand(GalacticManager galacticManager, Player player, SWGObject target, String args, boolean changeDance) {
		...
		if (args == null || args.isEmpty()) {
			...
			listBox.addOkButtonCallback("handleSelectedItem", (event, parameters) -> {
				...
				new com.projectswg.holocore.intents.DanceIntent(selectedDanceName, player.getCreatureObject(), changeDance).broadcast();
			});
			
			listBox.display(player);
		} else {
			new com.projectswg.holocore.intents.DanceIntent(args, player.getCreatureObject(), changeDance).broadcast();
		}
	}
}
