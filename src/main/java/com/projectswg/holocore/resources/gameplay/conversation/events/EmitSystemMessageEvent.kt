...
class EmitSystemMessageEvent(private val text: String) : Event {
	override fun trigger(player: Player, npc: AIObject) {
		val creatureObject = player.creatureObject ?: return
		SystemMessageIntent(player, text.replace("%TU", creatureObject.objectName)).broadcast()
	}
}
