...
class FactionNameRequirement(private val factionName: String) : Requirement {
	override fun test(player: Player): Boolean {
		val creatureObject = player.creatureObject
		val currentFaction = creatureObject.faction
		val requiredFaction = factions.getFaction(factionName)

		return currentFaction == requiredFaction
	}
}
