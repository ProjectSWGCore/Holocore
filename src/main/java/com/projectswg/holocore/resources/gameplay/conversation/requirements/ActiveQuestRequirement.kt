...
class ActiveQuestRequirement(val questName: String, val isActive: Boolean, val task: Int?) : Requirement {
	override fun test(player: Player): Boolean {
		...
		if (task != null) {
			...
			val taskActive = questActiveTasks.contains(task)

			return taskActive == isActive
		}
