...
class AdminSpawnerService : Service() {
	...
	private fun handleChatCommandIntent(eci: ExecuteCommandIntent) {
		if (eci.command.crc == COMMAND_CREATE_SPAWNING_ELEMENT) {
			...
			val building = creature.superParent?.buildoutTag ?: location.terrain.name.toLowerCase(Locale.US).substring(0, 3)
			...
			val output = "${location.terrain} $type $comment $building $cell ${location.x} ${location.y} ${location.z} ${location.yaw} ${System.lineSeparator()}"
