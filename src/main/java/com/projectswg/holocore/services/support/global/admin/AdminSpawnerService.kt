...
class AdminSpawnerService : Service() {
	...
	private fun handleChatCommandIntent(eci: ExecuteCommandIntent) {
		if (eci.command.crc == COMMAND_CREATE_SPAWNING_ELEMENT) {
			...
			val output = String.format("%s\t%s\t%s\t%s\t%d\t%.1f\t%.1f\t%.1f\t%.0f%s", location.terrain, type, comment, building, cell, location.x, location.y, location.z, 360 - location.yaw, System.lineSeparator())
