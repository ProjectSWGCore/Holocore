...
//		SWGObject vcdInventory = ObjectCreator.createObjectFromTemplate("TODO inventory template");
		...
		//	vcdInventory.moveToContainer(vehicleControlDevice);
...
//		ObjectCreatedIntent.broadcast(vcdInventory);
	...
	private void storePet(CreatureObject pet, SWGObject petControlDevice) {
		...
		switch (pet.getSlottedObject("inventory").moveToContainer(petControlDevice)) {
			case SUCCESS:
				...
				break;
			default:
				Log.d("Failed storing pet %s in control device %s", pet, petControlDevice);
				break;
