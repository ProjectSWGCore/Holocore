...
public class SitOnObjectCmdCallback implements ICmdCallback {
	...
	public void execute(ObjectManager objManager, Player player,SWGObject target, String args) {
		...
		if (args.length() > 0) {
			...
			sot = new SitOnObject(cellid,(long) objectID, x, y, z);			
			creature.setStatesBitmask(32768);
		} else {
			Location loc = creature.getLocation();
			sot = new SitOnObject(0,(long) objectID, (float) loc.getX(), (float) loc.getY(), (float) loc.getZ());
		}
		creature.setPosture(Posture.SITTING);
		creature.setTurnScale(0);
		creature.setMovementScale(0);
		creature.setAccelScale(0);
		
		//ObjectController cont = new ObjectController(SitOnObject.CRC, player.getCreatureObject().getObjectId(), sot, 27);
